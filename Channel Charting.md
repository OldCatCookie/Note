...
## 十三、Not-Too-Deep Channel Charting (N2D-CC)

### 摘要

信道图（CC）是一种新兴的机器学习方法，用于在多天线系统中学习信道状态信息（CSI）的低维表示，同时保持CSI样本之间的空间关系。CC的驱动目标是以完全无监督的方式学习这些表示或通道图，即无需访问明确的地理信息。基于最近在深度流形学习中的发现，本文通过深度流形学习的“不太深”（N2D）方法来解决CC问题。根据所提出的方法，首先使用基于深度神经网络（DNN）的自动编码器（AE）来学习全局通道图的嵌入，然后使用浅聚类方法来搜索该嵌入的底层流形。通过这种方式，我们能够解决末端塌陷的问题，这是通道制图方法的一个众所周知的缺陷，在以前的研究工作中，只有通过引入距离约束形式的侧信息才能缓解这种缺陷。为了进一步利用现代多天线系统中不断增长的时空CSI分辨率，我们建议使用卷积神经网络（CNN）输入层来增强所使用的AE。所得到的卷积自动编码器（CAE）架构能够从波束空间域CSI中自动提取稀疏分布的时空特征，从而降低所得到的模型的计算复杂度。

### 算法

本文的算法大致分为两个流程：

1）将自动编码器应用于原始CSI数据，以学习初始全局表示；

2） 使用保持全局和局部距离的流形学习方法重新嵌入自动编码的表示。

### 自动编码器

先做CSI提取：

$$s_{i,j}(\mathbf{H}^{(t_1)},\ldots,\mathbf{H}^{(t_T)})=\frac1T\sum_{t\in\mathcal{T}}|\bar{\mathbf{H}}^{(t)}(i,j)-\hat{\mu}_{i,j}|^2,$$

![](image/20231204193854.png)

其中网络可以是DNN或是CNN。

### UMAP

UMAP的操作过程分为两步:在初始步骤中，它在原始高维空间中构造加权k近邻图，然后进行第二步优化，在较低维空间中找到最相似的图。通过搜索具有最接近等价模糊拓扑结构的数据的较低维表示来找到嵌入。在数据集中的两个样本之间选择一个相似度度量，它反映了样本之间的一些局部属性，如邻近样本的密度。UMAP还提供了稳定的样本外扩展，使其适合CC。

---

## 十四、Toward Massive MIMO 2.0: Understanding Spatial Correlation, Interference Suppression, and Pilot Contamination

### 摘要

自2010年Marzetta发表开创性论文以来，大规模MIMO已经从一个具有无限数量天线的理论概念转变为一种实用技术。关键概念被纳入5G新无线电标准，具有M=64个全数字收发器的基站（BS）已被商业部署在低于6 GHz的频带中。这一快速进展得益于许多扎实的研究成果，其中绝大多数都假设了为单细胞操作开发的空间不相关信道和信号处理方案。这些假设使得大规模MIMO的性能分析和优化变得容易处理，但有三个主要注意事项：1）实际信道在空间上是相关的；2） 可以在没有BS协作的情况下通过多小区处理获得大的性能增益；3） 导频污染引起的干扰产生了一个有限的容量极限，如M→ ∞.有一小部分论文避免了这些警告，但结果很容易被遗漏。因此，本教程文章解释了考虑空间信道相关性和使用为多小区网络设计的信号处理方案的重要性。我们介绍了大规模MIMO的基本极限的最新结果，这些极限不是由导频污染决定的，而是由获取信道统计信息的能力决定的。这些结果将引导我们迈向下一个大规模MIMO水平，我们称之为“大规模MIMO 2.0”。
